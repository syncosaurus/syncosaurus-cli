...
export default class Reflect {
	constructor(options) {
		...
		this.localState = {}; //create a KV store instance for the reflect client that serves as UI display
		this.canonState = {}; //create a KV store instance for the reflect client that serves as source of truth after server runs
		this.txQueue = []; // create a tx

		// establish websocket connection with DO
		// this.socket = new WebSocket('ws://localhost:8787/websocket');
		// wss://websocket-server.sklarfox.workers.dev/websocket
		// wss://websocket-server.josephliang.workers.dev/websocket
		this.socket = new WebSocket('wss://websocket-server.josephliang.workers.dev/websocket');
		...
		this.socket.addEventListener('message', (event) => {
			...
			this.txQueue = this.txQueue.filter((tx) => tx.transactionID !== response.transactionID);
    ...
    */
	...
	unsubscribe(query, callback) {
		if (this.subscriptions[query]) {
			this.subscriptions[query] = this.subscriptions[query].filter((cb) => cb !== callback);
		}
	}
